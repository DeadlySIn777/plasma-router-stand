...
BASELINE='a61d63453f2c2e75dc06472d187da3702e433fcb'
REPORT=Path('output/design-completion-2026-09-26/mirror-verification.json')
...
def in_scope(name):
    return (name in {'README.md','build_concept_revh.py','package_revh.py','sync_revh_workspace.py',
                    'output/pdf/plasma-router-stand-concept.pdf',
                    'output/cad-repair-2026-09-25/SUPPLIER-DRAWING-REQUEST.md'}
            or name.startswith(('output/design-completion-2026-09-26/',
                                'output/release-review/RevH-CAD/'))
            or name in {'output/release-review/RevE-ENGINEERING/'+p for p in (
                'build_revh.py','bed_completion.py','water_completion.py','motion_completion.py','render_revh.py')})
...
def main():
    ...
    paths.update(str(p.relative_to(ROOT)).replace('\\','/')
                 for p in (ROOT/'output/release-review/RevH-CAD/previews').glob('*.npz'))
