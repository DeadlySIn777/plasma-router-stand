"""Check the current Rev I inventory and regenerate its material layouts.
...
EVIDENCE=ROOT/'output/design-finish-2026-09-26'
...
def main():
    ...
    validation=json.loads((CAD/'RevI_ROUTER-validation.json').read_text(encoding='utf-8'))
    panel_mass=[0.0]*6
    ...
    bound=[Path(__file__).resolve(),CAD/'engineering-manifest.json',CAD/'cutlist.json',CAD/'part-operations.json',
           CAD/'RevI_ROUTER-validation.json',CAD/'tube-cut-plan.json']
    bound.extend(CAD/'parts'/name for name in sorted(expected_parts))
    bound.extend(CAD/'dxf'/name for name in sorted(expected_flats))
    report['artifact_sha256']={str(p.relative_to(ROOT)).replace('\\','/'):hashlib.sha256(p.read_bytes()).hexdigest() for p in bound}
