...
static void arcSetState (spindle_ptrs_t *spindle, spindle_state_t state, float rpm)
{
    ...
    if(!state.on) {
        ...
        // Kraken engineering port: a stored zero must not underflow --retries.
        // One means one initial attempt with no automatic retry.
        uint_fast8_t retries = plasma.arc_retries ? plasma.arc_retries : 1;
...
PROGMEM static const setting_detail_t plasma_settings[] = {
    ...
    { Setting_Arc_MaxRetries, Group_Plasma, "Plasma Arc max retries", NULL, Format_Int8, "#0", "1", "255", Setting_NonCore, &plasma.arc_retries, NULL, NULL },
...
static void plasma_settings_restore (void)
{
    static const plasma_settings_t default_settings = {
        ...
        .arc_retries = 1, // One attempt, no automatic re-strike by default.
