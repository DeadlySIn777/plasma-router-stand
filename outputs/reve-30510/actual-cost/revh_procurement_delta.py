"""What GM1 Rev H changes in the shopping list, computed from the two cut lists.

Compares output/release-review/RevG-CAD/cutlist.json with RevH-CAD/cutlist.json
part number by part number, then the two tube plans and the two stock-fit
reports (check_revg_stock_fit.py with --cad). Writes revh-procurement-delta.json.
Stdlib only.
"""
...
def main():
    ...
    changed = [{'part_number': p, 'rev_g_quantity': g[p]['quantity'], 'rev_h_quantity': h[p]['quantity'], 'material': h[p]['material']}
    ...
    fit = {}
    for rev, name in (('g', 'revg-stock-fit.json'), ('h', 'revh-stock-fit.json')):
    ...
    (HERE / 'revh-procurement-delta.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
