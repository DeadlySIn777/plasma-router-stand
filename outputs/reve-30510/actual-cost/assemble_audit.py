...
tubeplan=read(HERE.parents[2]/'output/release-review/RevJ-CAD/tube-cut-plan.json')
assert tubeplan['stock_qty']==6 and len(tubeplan['bars'][5]['cuts'])==2 and all(c['part'].startswith('MOD_CROSSMEMBER') for c in tubeplan['bars'][5]['cuts']), 'Rev J tube plan changed: review the MET01 / MET-GAP-23 text'
...
for i,r in enumerate(metal['quote_only_raw_stock_schedule'],1):
    ...
    if r.get('superseded'):
        superseded.append(dict(id=f'MET-GAP-{i}',item=r['item'],quantity=r['quantity'],unit_usd=None,goods_usd=None,url='',reason=r['superseded'],revision='J'));continue
...
for r in hardware['offers']:
    ...
    if r.get('superseded_by_rev_g') or r.get('superseded_by_rev_j'):
        rev='J' if r.get('superseded_by_rev_j') else 'G'
        superseded.append(dict(id='HW-'+r['id'],item=r['item'],quantity=r['buy_quantity'],unit_usd=r['unit_usd'],goods_usd=r['goods_usd'],url=r['url'],reason=r.get('superseded_by_rev_j') or r['superseded_by_rev_g'],revision=rev));continue
...
gap('J-GALV','Hot-dip galvanizing, Rev J bed module','About 32 kg of steel: rails, crossmembers, end plates, caps, lugs and sleeves welded as one module. Vent and drain holes as agreed with the galvanizer; a minimum lot charge usually governs. Unquoted.',1)
...
nuttybasis=('Goods are above $100 and qualify for free shipping.' if nuttyship==0 else f'Goods ${nuttygoods:,.2f} are below the $100 free-shipping threshold, so the published $10.95 flat rate applies unless the unpriced Rev J stainless fasteners are added to this order.')
...
if marketplace_path.exists():
    ...
    if mp.get('five_24ft_fit'):
        options.append(dict(id='FB-SYLVANIA',item='Marketplace Sylvania: 2 x 2 x 24 ft tube, seller describes 1/8 in wall',quantity=5,unit_price=115,currency='USD',extended=575,url='https://www.facebook.com/marketplace/item/1367277908682890/',reason='Seller advertises $115 each and 28 available; pickup in Sylvania GA. The recorded five-bar cut proof was made for the Rev E 36-blank schedule; the current Rev J 32-blank schedule has not been re-proved for 24 ft bars, so the quantity shown is that superseded proof. Grade, actual wall (.120 versus nominal1/8), rust/coating, straightness, quantity and collection cost unverified. Conditional option only; no adopted savings.'))
...
sup_g=[r for r in superseded if r['revision']=='G']; sup_j=[r for r in superseded if r['revision']=='J']
...
summary=dict(date='2026-09-24',currency='USD',destination='Alto GA30510',status='PARTIAL PRICE AUDIT — COMPLETE BUILD TOTAL UNRESOLVED',
    design_revision='GM1 Rev J working design: one-piece bed module with the Rev H water service and Z adapter (26 Sep 2026)',
    ...
    orders_placed_by_agent=0,owner_reported_orders=[r['id'] for r in rows if r.get('owner_order_status','').startswith('ordered')],quoted_8020_order_including_estimated_tax_usd=272.11,
...
lines += ['',f"The current register contains **{len(rows)} priced lines**, **{len(gaps)} remaining scope entries** and a vendor-level shipping register. Every price is linked to a product or supplier. Line quantities are rounded to cents; seller checkout can differ by a cent on fractional-cent fasteners.", '',
    ...
    ('Nutty hardware is combined into one order above its $100 advertised free-shipping threshold.' if nuttyship==0 else f'Nutty hardware is combined into one order of ${nuttygoods:,.2f}, below its $100 free-shipping threshold, so the $10.95 flat rate is included; adding the unpriced Rev J stainless fasteners to this order may remove it.')+' No duplicate delivery charge is added for each fastener row. Other blank freight cells remain unknown.', '',
    *(['**Owner order status:** '+'; '.join(f"{r['id']} {r['item']}, {r['owner_order_status']}" for r in rows if r.get('owner_order_status'))+'. Listed prices stay in this register until the price paid is recorded. See the [drive-module receiving check](../../../output/receiving/MOTION-MODULES.md).', ''] if any(r.get('owner_order_status') for r in rows) else []),
    ...
    "GM1 Rev J (`output/release-review/RevJ-CAD/`) is the owner's one-piece, hoist-lifted, waterproof bed module, first published on this branch as Rev H, combined with the base branch's Rev H water service (reservoir hatches, washout flange, drain reserves) and Z-adapter blank. Its refill spout is moved clear of the module. The base branch's own Rev H keeps the six-panel bed and is not priced here. "+f"The bed rows now follow the Rev J cut list. {len(sup_j)} Rev G bed items left the register and stay as dated evidence (`superseded_offers`, revision J): "+', '.join(r['id']+(f" ${r['goods_usd']:,.2f}" if r['goods_usd'] is not None else ' (was unpriced)') for r in sup_j)+'. N1 is now 48 stainless square nuts, and B01 stays at five two-packs (nine 1,197 mm profiles plus one spare bar).', '',
