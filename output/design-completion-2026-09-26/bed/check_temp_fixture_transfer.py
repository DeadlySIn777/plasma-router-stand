"""Full Rev H bare temporary C-frame transfer; hardware and hand paths excluded."""
from pathlib import Path
import sys,os,json,hashlib
from check_restraint_paths import SOURCE,ROOT,transform,bounds_speed,swept_box,continuous_segment,build_model,clone_model,store_router_tool,bbox,h,bc
OUT=Path(__file__).with_name('temporary-fixture-transfer.json')

def hashes():
    paths=list(SOURCE.glob('*.py'))+[Path(__file__),Path(__file__).with_name('check_restraint_paths.py'),ROOT/'output/cad-repair-2026-09-25/check_spoil_transfer.py']
...
def main():
    ...
    report={'scope':__doc__,'source_sha256':before,'model_parts':len(temporary.parts),'paths':[],
