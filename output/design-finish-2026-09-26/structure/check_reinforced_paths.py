"""Reinforced rod handling against the selected complete model; no fingers or small-screw path claim."""
...
from build_revh import build_model as base_build
import structure_finish
def build_model(*args,**kwargs):
    if "--integrated" in sys.argv:
        from build_revi import build_model as final_build
        return final_build(*args,**kwargs)
    m,d=base_build(*args,**kwargs);d["structure_finish"]=structure_finish.extend_router_model(m);return m,d
...
OUT=Path(__file__).with_name('reinforced-restraint-paths.json')
...
def main():
    ...
    report={'scope':__doc__,'builder':'build_revi.build_model' if '--integrated' in sys.argv else 'build_revh plus structure_finish','source_sha256':before,'model_parts':len(router.parts),'paths':[],
    ...
    for side,x in enumerate((315,835),1):
        ...
        ops=[('T',(0,0,60)),('T',(360-x,0,0)),('T',(0,292,0)),
             ('R',(360,402,235),(361,402,235),90),('T',(0,0,316.596)),
             ('T',(0,stagey+overshoot-402,0)),('T',(stagex-360,0,0)),('T',(0,-overshoot,0)),('T',(0,0,-8.5))]
        hoop_ops[side]=ops
