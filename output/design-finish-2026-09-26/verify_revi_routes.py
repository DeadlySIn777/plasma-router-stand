"""Run unchanged Rev G continuous large-part solvers against full Rev I.

Builder injection changes the actual obstacles and moved components. Each phase
uses the defined Rev I restraint state. This is not a proof of small-part moves,
...
def run(kind):
    ...
    old_module = load('revi_reused_' + kind, script)
    record = {}

    def completed_builder(*args, **kwargs):
        model, details = build_revi.build_model(*args, **kwargs)
        prepare(model)
    ...
    # A module proxy supplies only the panel import. build_revi.previous still
    ...
    result['revision'] = 'Rev I full integrated machine, original prescribed large-part route'
    result['builder'] = 'build_revi.build_model'
    result['revi_obstacle_state'] = record
    ...
    print('REVI_ROUTE', kind, 'PASS' if result['integrated_passed'] else 'FAIL',
