...
def render(name,view=(-1.25,-1.8,1.25),size=(1600,1400),suffix=''):
    ...
    titles={'RevG_ROUTER':'Rev G | Router bed installed',
            'RevG_BED_STORED':'Rev G | Bed and router tools stored inside',
            'RevG_STORAGE_DETAIL':'Rev G | Storage and service arrangement'}
    ...
    draw.text((42,h-77),'WORKING REPAIR: manual conversion assumption; supplier interfaces still open.',font=font,fill=(112,66,18))
    note='The stored-bed view does not include a verified plasma head.' if name!='RevG_ROUTER' else 'Nominal motion: X 800 / Y 1000 / Z 100 mm. Not a fabrication release.'
...
def main():
    results=[]
    for name in ('RevG_ROUTER','RevG_BED_STORED'):
        print('Render',name,flush=True);results.append(render(name))
    results.append(render('RevG_BED_STORED',view=(0,-1,.18),size=(1400,1400),suffix='_front'))
