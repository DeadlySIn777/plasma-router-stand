"""Check nine nominal full-machine Rev G router poses from fresh source CAD.

Checks all eight X/Y/Z travel-box corners and the center. Each includes the
fixed chassis, actual frame-raised cap blanks, bed, water and packaging geometry.
...
DEFAULT_OUTPUT=SOURCE.parents[1]/'cad-repair-2026-09-25'/'motion'/'revg-full-machine-poses.json'
...
def main(output=DEFAULT_OUTPUT):
    ...
    report={'scope':__doc__,'revision':'Rev G working repair','status':'RUNNING',
    ...
    try:
        ...
        print('Rev G fixed assembly:',len(fixed.parts),'parts',flush=True)
        for gy,hx,z in STATES:
            ...
            write_report(output,report,start)
            print('Rev G pose',gy,hx,z,'parts',result['part_count'],
        ...
        write_report(output,report,start)
        print('Rev G full-machine motion:',report['status'],len(report['states']),
