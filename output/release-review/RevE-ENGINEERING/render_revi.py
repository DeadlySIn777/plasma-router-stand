...
def render(name,view=(-1.25,-1.8,1.25),size=(1600,1400),suffix=''):
    ...
    titles={'RevI_ROUTER':'Rev I | Router bed installed',
            'RevI_BED_STORED':'Rev I | Bed and router tools stored inside',
            'RevI_PLASMA_HARDWARE':'Rev I | Floating / breakaway head hardware; actual torch absent'}
    ...
    note='New head hardware is modeled. Actual torch and purchased interfaces remain unqualified.' if name!='RevI_ROUTER' else 'Nominal motion: X 800 / Y 1000 / Z 100 mm. Not a fabrication release.'
...
def main():
    results=[]
    for name in ('RevI_ROUTER','RevI_BED_STORED','RevI_PLASMA_HARDWARE'):
        print('Render',name,flush=True);results.append(render(name))
    results.append(render('RevI_BED_STORED',view=(0,-1,.18),size=(1400,1400),suffix='_front'))
